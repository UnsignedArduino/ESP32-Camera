...
void ArduCAM::setSpiBus(SPIClass* bus) { this->spiBus = bus; }
...
void ArduCAM::set_fifo_burst() {
  ...
  this->spiBus->transfer(BURST_FIFO_READ);
#endif
}
...
uint8_t ArduCAM::bus_write(int address, int value) {
  ...
  // SPI.transfer(value);
  this->spiBus->transfer(address);
  this->spiBus->transfer(value);
#endif
  sbi(P_CS, B_CS);
  return 1;
}
...
uint8_t ArduCAM::bus_read(int address) {
  ...
  // value = SPI.transfer(0x00);
  this->spiBus->transfer(address);
  value = this->spiBus->transfer(0x00);
  // take the SS pin high to de-select the chip:
  sbi(P_CS, B_CS);
  return value;
#endif
